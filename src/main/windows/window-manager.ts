...
export class WindowManager {
	...
	createMainWindow(): BrowserWindow {
		...
		const x = Math.min(
			Math.max(state.position.x, workArea.x),
			workArea.x + workArea.width - state.size.width,
		);
		const y = Math.min(
			Math.max(state.position.y, workArea.y),
			workArea.y + workArea.height - state.size.height,
		);
		...
		this.mainWindow = new BrowserWindow({
			...
			trafficLightPosition: { x: 10, y: 10 }, // Show traffic lights with proper spacing
			webPreferences: {
		...
		if (this.isDev) {
			...
			this.mainWindow.loadFile(
				path.join(__windowDirname, "../renderer/index.html"),
			);
		}
