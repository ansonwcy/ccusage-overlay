...
export function UsageSummary({ title, data, showProgress = false, maxValue }: UsageSummaryProps) {
	if (!data) {
		return (
	...
	const totalTokens = "tokens" in data 
		? data.tokens.inputTokens + data.tokens.outputTokens + data.tokens.cacheCreationTokens + data.tokens.cacheReadTokens
		: data.inputTokens + data.outputTokens + data.cacheCreationTokens + data.cacheReadTokens
	
	const percentChange = "percentChange" in data ? data.percentChange : undefined
				...
				<div className="w-full h-2 bg-[var(--bg-tertiary)] rounded-full mb-2 overflow-hidden">
					<div 
						className={`h-full transition-all duration-300 ${
							progress > 90 ? "bg-danger" : progress > 75 ? "bg-warning" : "bg-primary"
						}`}
