...
export function ProjectBreakdown({ projects, maxItems = 3 }: ProjectBreakdownProps) {
	if (!projects || projects.length === 0) {
		return (
			<div className="p-4">
				<h3 className="text-sm font-medium text-[var(--text-secondary)] mb-3">Top Projects</h3>
				<p className="text-xs text-[var(--text-secondary)]">No project data available</p>
			</div>
		)
	}
				...
				{topProjects.map((project, index) => (
					<div key={project.project} className="flex items-center justify-between">
						<div className="flex items-center space-x-2 flex-1 min-w-0">
							<div className={`w-3 h-3 rounded-sm ${PROJECT_COLORS[index % PROJECT_COLORS.length]}`} />
